...
//! The `fxsave + fxrstor` target feature.
...
unsafe impl TargetFeatureToken for Fxsr {
    ...
    fn vectorize<R>(self, f: impl FnOnce() -> R) -> R {
        ...
        trampoline!([Fxsr = self] => "fxsr", <(R)> fn<(R)>(f: impl FnOnce() -> R = f) -> R { f() })
    }
}
...
impl Fxsr {
    ...
    #[target_feature(enable = "fxsr")]
    /// Create a new token for the "fxsr" target feature.
    ///
    /// This method is useful to get a new token if you have an external proof that
    /// `fxsave + fxrstor` is available. This could happen if you are in a target feature
