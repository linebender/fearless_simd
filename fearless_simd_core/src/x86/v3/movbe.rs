...
unsafe impl TargetFeatureToken for Movbe {
    ...
    fn vectorize<R>(self, f: impl FnOnce() -> R) -> R {
        ...
        trampoline!([Movbe = self] => "movbe", <(R)> fn<(R)>(f: impl FnOnce() -> R = f) -> R { f() })
    }
}

impl Movbe {
    #[cfg(feature = "std")]
    /// Create a new token if the `"movbe"` target feature is detected as enabled.
    ///
