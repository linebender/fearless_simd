...
unsafe impl TargetFeatureToken for Xsave {
    ...
    fn vectorize<R>(self, f: impl FnOnce() -> R) -> R {
        ...
        trampoline!([Xsave = self] => "xsave", <(R)> fn<(R)>(f: impl FnOnce() -> R = f) -> R { f() })
    }
}

impl Xsave {
    #[cfg(feature = "std")]
    /// Create a new token if the `"xsave"` target feature is detected as enabled.
    ///
