...
///
/// A token indicating that the current CPU has the `sha` target feature.
///
/// # Example
///
...
unsafe impl TargetFeatureToken for Sha {
    ...
    fn vectorize<R>(self, f: impl FnOnce() -> R) -> R {
        ...
        trampoline!([Sha = self] => "sha", <(R)> fn<(R)>(f: impl FnOnce() -> R = f) -> R { f() })
    }
}

impl Sha {
    #[cfg(feature = "std")]
    /// Create a new token if the `"sha"` target feature is detected as enabled.
    ///
...
impl From<Sha> for crate::x86::sse::Sse {
    fn from(value: Sha) -> Self {
        // This also serves as a correctness check of the implicitly enabled features.
        trampoline!([Sha = value] => "sha", fn() -> crate::x86::sse::Sse { crate::x86::sse::Sse::new() })
    }
}

impl From<Sha> for crate::x86::sse::Sse2 {
    fn from(value: Sha) -> Self {
        // This also serves as a correctness check of the implicitly enabled features.
        trampoline!([Sha = value] => "sha", fn() -> crate::x86::sse::Sse2 { crate::x86::sse::Sse2::new() })
    }
}

const _: () = {
    assert!(
        core::mem::size_of::<Sha>() == 0,
