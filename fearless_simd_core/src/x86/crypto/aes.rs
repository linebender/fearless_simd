...
///
/// A token indicating that the current CPU has the `aes` target feature.
///
/// # Example
///
...
unsafe impl TargetFeatureToken for Aes {
    ...
    fn vectorize<R>(self, f: impl FnOnce() -> R) -> R {
        ...
        trampoline!([Aes = self] => "aes", <(R)> fn<(R)>(f: impl FnOnce() -> R = f) -> R { f() })
    }
}

impl Aes {
    #[cfg(feature = "std")]
    /// Create a new token if the `"aes"` target feature is detected as enabled.
    ///
...
impl From<Aes> for crate::x86::sse::Sse {
    fn from(value: Aes) -> Self {
        // This also serves as a correctness check of the implicitly enabled features.
        trampoline!([Aes = value] => "aes", fn() -> crate::x86::sse::Sse { crate::x86::sse::Sse::new() })
    }
}

impl From<Aes> for crate::x86::sse::Sse2 {
    fn from(value: Aes) -> Self {
        // This also serves as a correctness check of the implicitly enabled features.
        trampoline!([Aes = value] => "aes", fn() -> crate::x86::sse::Sse2 { crate::x86::sse::Sse2::new() })
    }
}

const _: () = {
    assert!(
        core::mem::size_of::<Aes>() == 0,
