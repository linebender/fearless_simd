...
///
/// A token indicating that the current CPU has the `kl` target feature.
///
/// # Example
///
...
unsafe impl TargetFeatureToken for Keylocker {
    ...
    fn vectorize<R>(self, f: impl FnOnce() -> R) -> R {
        ...
        trampoline!([Keylocker = self] => "kl", <(R)> fn<(R)>(f: impl FnOnce() -> R = f) -> R { f() })
    }
}

impl Keylocker {
    #[cfg(feature = "std")]
    /// Create a new token if the `"kl"` target feature is detected as enabled.
    ///
    ...
    pub fn try_new() -> Option<Self> {
        ...
        #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
        if std::arch::is_x86_feature_detected!("kl") {
