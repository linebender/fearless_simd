...
pub(crate) fn mk_simd_trait() -> TokenStream {
    ...
    let mut code = quote! {
        ...
        pub trait Simd: Sized + Clone + Copy + Send + Sync + Seal + arch_types::ArchTypes + 'static {
            ...
            type f32s: SimdFloat<Self, Element = f32, Block = f32x4<Self>, Mask = Self::mask32s, Bytes = Self::u8s> + SimdCvtFloat<Self::u32s> + SimdCvtFloat<Self::i32s>;
            ...
            type i8s: SimdInt<Self, Element = i8, Block = i8x16<Self>, Mask = Self::mask8s, Bytes = Self::u8s> + core::ops::Neg<Output = Self::i8s>;
            ...
            type i16s: SimdInt<Self, Element = i16, Block = i16x8<Self>, Mask = Self::mask16s, Bytes = Self::u8s> + core::ops::Neg<Output = Self::i16s>;
            /// A native-width SIMD vector of [`u32`]s.
            type u32s: SimdInt<Self, Element = u32, Block = u32x4<Self>, Mask = Self::mask32s, Bytes = Self::u8s> + SimdCvtTruncate<Self::f32s>;
            /// A native-width SIMD vector of [`i32`]s.
            type i32s: SimdInt<Self, Element = i32, Block = i32x4<Self>, Mask = Self::mask32s, Bytes = Self::u8s> + SimdCvtTruncate<Self::f32s>
