...
mod tests {
    ...
    mod neon_kernel {
        ...
        fn neon_kernel() {
            if let Some(neon) = Level::new().as_neon() {
                let _ = splat_u32(neon, 42);
            }
        }
    }

    #[cfg(all(target_arch = "wasm32", target_feature = "simd128"))]
    mod wasm_simd128_kernel {
        use crate::Level;
        use core::arch::wasm32::{u32x4_splat, v128};

        crate::wasm_simd128_kernel! {
            #[inline]
            fn splat_u32(x: u32) -> v128 {
                u32x4_splat(x)
            }
        }

        #[test]
        fn wasm_simd128_kernel() {
            let wasm = Level::new()
                .as_wasm_simd128()
                .expect("simd128 is statically enabled");
            let _ = splat_u32(wasm, 42);
        }
    }
    ...
    mod x86_kernel {
        ...
        use core::arch::x86_64::{__m256i, _mm_crc32_u32, _mm256_setzero_si256};
