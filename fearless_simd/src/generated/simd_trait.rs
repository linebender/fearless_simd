...
use crate::{
    Bytes, ExtractToken, Level, MaskNarrow, MaskWiden, Select, SimdCvtFloat, SimdCvtTruncate,
    SimdElement, SimdFloatElement, SimdFrom, SimdIntElement, SimdInto, SimdNarrow, SimdWiden,
    seal::Seal,
};
use crate::{
    ...
    + 'static
{
    ...
    fn level(self) -> Level;
    #[doc = r" Call a closure with the instructions for this SIMD level enabled."]
    #[doc = r""]
    #[doc = r" `vectorize()` establishes the correct `#[target_feature]` context. The provided"]
    #[doc = r" closure should be `#[inline(always)]` so that its body is incorporated into that"]
    #[doc = r" context and can use all instructions supported by this SIMD level."]
    #[doc = r""]
    #[doc = r" The small wrappers used by `vectorize()` are eligible for inlining when the caller"]
    #[doc = r" has a compatible target-feature context. A function containing the call does not"]
    #[doc = r" itself need an `#[inline(always)]` annotation; a target-feature boundary is retained"]
    #[doc = r" whenever one is required."]
    #[doc = r""]
    #[doc = r" The [`#[simd]`](https://docs.rs/fearless_simd_macros/latest/fearless_simd_macros/attr.simd.html)"]
    #[doc = r" attribute from the separate `fearless_simd_macros` crate establishes this context"]
    #[doc = r" automatically, passing ordinary function arguments separately to reduce capture"]
    #[doc = r" overhead. Calling `vectorize()` manually avoids the procedural-macro dependency."]
    #[doc = r""]
    #[doc = r" # Example"]
    #[doc = r""]
    #[doc = r" `double_u32s` does not need to be marked `#[inline(always)]`; only the closure passed"]
    #[doc = r" to `vectorize()` does."]
    #[doc = r""]
    #[doc = r" ```"]
    ...
    #[doc = "Narrow each boolean lane of two masks and concatenate them into one mask of the same logical width, preserving each value.\n\n`a` supplies the lower result lanes and `b` supplies the upper result lanes. Masks constructed from integer lanes other than 0 or -1 have unspecified behavior."]
