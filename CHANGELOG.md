...
- Added `Native<S>` to `SimdFloatElement` and `SimdIntElement` to select native-width vectors in scalar-generic code. The new `sigmoid_generic` example demonstrates dispatch and partial-vector handling for both `f32` and `f64`.
