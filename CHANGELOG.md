...
- Breaking change: the 204 vector-specific `Simd` array conversion methods have been replaced by the generic `load_array`, `load_array_ref`, `as_array`, `as_array_ref`, `as_array_mut`, and `store_array` methods. The shared `SimdArray` trait now also owns the lane `Element` type, `N`, `from_slice`, and `store_slice` for both ordinary vectors and masks.
